...
foreach ($Proc in $Process){
    ...
    ($Proc | select ProcessName, ProcessID, @{n="ParentProcessName";e={ $ParentProcessName }}, ParentProcessID, Path, CommandLine, @{n="Modules";e={((get-process | ? {$_.id -eq $proc.processid}).modules).filename -join "; "}} | Format-List | Out-String).trim()
